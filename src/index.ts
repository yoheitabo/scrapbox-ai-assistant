...
import { ProjectConfig, type ScrapboxConfig } from "./types.js";
...
async function main() {
  ...
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      if (config.projects) {
        for (const projectConfig of config.projects) {
          if (projectConfig.exportPaths && projectConfig.exportPaths.length > 0) {
            // 複数の分割ファイルから読み込み
            console.error(
              `Loading project ${projectConfig.name} from ${projectConfig.exportPaths.length} split files...`,
            );
      ...
      console.error(
        "Create a config file with project settings, or set SCRAPBOX_CONFIG_PATH environment variable",
      );
